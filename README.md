...
## 🛡️ AGI Prospection Suite (Agency Extensions)

This fork includes specialized tools for high-performance B2B sales:

- **📊 Dataset Ingestion:** Native support for Google Places & Apify JSON datasets with auto-ICP scoring.
- **📲 Telegram Alerts:** Real-time push notifications for high-priority leads.
- **📞 Twilio Outreach:** Integrated SMS and Voice call automation.
- **🎯 Elite Frameworks:** Built-in scripts for Charlie Morgan, Aaron Shepherd, and Jordan Platten methodologies.

### 🛠️ Quick Deployment

We provide unified scripts for easy startup on Windows:

- **Docker Mode (Recommended):** Double-click `deploy.bat` or run `.\deploy.ps1`.
- **Local Mode:** Run `.\deploy.ps1 -Mode local`.
